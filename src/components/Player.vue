  ...
  <div class="playerWrapper" v-if="!!playerCurrentTrack">
    ...
    <div class="mainButtons">
      ...
      <button @click="$store.dispatch(isPlay ? 'pause': 'play')" class="playButton">
        <img :src="isPlay ? require('../assets/icons/pause.svg') : require('../assets/icons/play.svg')" alt="">
    ...
    <div class="seekbarWrapper">
      ...
      <div class="seekbar" @click="calculateSeekOnClick">
        ...
        <div class="seek" :style="{width: `${(((playerCurrentTime / playerDuration ) * 100) || 0)}%`}"></div>
        <div class="seekbarPoint" :style="{left: `calc(${(((playerCurrentTime / playerDuration) * 100 || 0))}% - 7px)`}"></div>
      </div>
      <span class="durationTime">{{secondsToTime(playerDurationd)}}</span>
    </div>
...
export default {
  ...
  watch: {
    ...
    playerCurrentTime(nextPlayerCurrentTime , prevPlayerCurrentTime){
      if(nextPlayerCurrentTime !== prevPlayerCurrentTime && this.playerSeeking){
        ...
        this.$store.dispatch('setPlayerSeeeking' , false)
      }
    },
  ...
  methods: {
    ...
    calculateSeekOnClick(e){
      ...
      const percent = 100 - (((element.clientWidth - (e.pageX - offset.left)) / element.clientWidth) * 100)
