<template>
  <div :class="`itemWrapper${main ? 'main' : ''}`">
    <div :style="`backgroundImage: url(${trackData.artwork_url}`" class="artwork">

      <div :class="`playOverlay${(playerCurrentTrack && (playerCurrentTrack.id === trackData.id)) && !isPlay ? 'active' : ((playerCurrentTrack && trackData && (playerCurrentTrack.id === trackData.id)) && isPlay) ? 'playing' : ''}`" @click="onPlayTrack(trackData, main)">
        <font-awesome-icon icon="play"/>
      </div>
      <div :class="`pauseOverlay${(playerCurrentTrack && trackData && (playerCurrentTrack.id === trackData.id)) && isPlay ? 'active' : ''}`" @click="onPlayTrack(trackData , main)">
  ...
  <div :class="`waveformWrappers${(playerCurrentTrack && (playerCurrentTrack.id === trackData.id)) ? 'playing' : ''}`"
  @mouseover="handleHoverWave" @click="handleClickWave">
  </div>
  <div class="waveform" :style="`background-imgae: url(${trackData.waveform_url})`" @click="onPlayTrack(trackData,main)">
    <div class="playOverlay">
      <font-awesome-icon icon="play" class="play" />
    </div>
    <div class="seekBackground" :style="`width: ${waveWidth}px`"></div>
    <div class="seekLine" :style="`width: ${waveWidth}px`"></div>
    <div class="playedBackground" :style="`width: ${playerCurrentTime / (playerDuration / 100)}%`"></div>
  </div>
</template>
