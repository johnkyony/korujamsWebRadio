<template>
  <div class="container">
    ...
    <div class="wrapper">
      <div :style="`backgroundImage: url(${trackData.artwork_url})`"
      :class="`artwork${(playerCurrentTrack && (playerCurrentTrack.id === trackData.id)) ? 'active' : ''}`" @click="onClick(trackData)">

        <div class="playOverlay">
          <img v-if="isPlay" class="playing" src="@/assets/playing.gif" alt="">
          <font-awesome-icon icon="stop-circle" class="stop"></font-awesome-icon>
          <font-awesome-icon icon="play-circle" class="play"></font-awesome-icon>
        </div>
      </div>
